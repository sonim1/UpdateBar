...
final class StateStoreTests: XCTestCase {
    ...
    func testStateStoreProvidesCrossProcessLockFileForReadModifyWrite() throws {
        let root = try temporaryDirectory()
        let store = StateStore(paths: AppPaths(homeDirectory: root))
        let state = State(schemaVersion: 1, generatedAt: Date(timeIntervalSince1970: 1_800), items: [:])

        try store.withExclusiveLock {
            try store.save(state)
        }

        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("state.lock").path))
