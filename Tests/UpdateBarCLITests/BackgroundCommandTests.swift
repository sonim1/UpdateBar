    ...
    final class BackgroundCommandTests: XCTestCase {
        func testBackgroundInstallWritesCheckOnlyLaunchAgent() throws {
            ...
            try FileManager.default.createDirectory(
                at: updatebarHome, withIntermediateDirectories: true)
        ...
        func testBackgroundStatusAndUninstall() throws {
            ...
            let missing = try CLIProcess.run(
                ["background", "status", "--json"], home: home, environment: ["HOME": home.path])
            XCTAssertEqual(missing.exitCode, 0)
            XCTAssertTrue(missing.stdout.contains(#""installed":false"#))

            _ = try CLIProcess.run(
                ["background", "install", "--yes"], home: home, environment: ["HOME": home.path])
            let installed = try CLIProcess.run(
                ["background", "status", "--json"], home: home, environment: ["HOME": home.path])
            XCTAssertEqual(installed.exitCode, 0)
            XCTAssertTrue(installed.stdout.contains(#""installed":true"#))

            let uninstall = try CLIProcess.run(
                ["background", "uninstall", "--json"], home: home, environment: ["HOME": home.path])
        ...
        func testBackgroundActionJSONIncludesLaunchAgentLabel() throws {
            ...
            let install = try CLIProcess.run(
                ["background", "install", "--yes", "--json"], home: home,
                environment: ["HOME": home.path])
            XCTAssertEqual(install.exitCode, 0)
            XCTAssertEqual(
                try jsonObject(in: install.stdout)["label"] as? String, "com.updatebar.check")

            let uninstall = try CLIProcess.run(
                ["background", "uninstall", "--json"], home: home, environment: ["HOME": home.path])
            XCTAssertEqual(uninstall.exitCode, 0)
            XCTAssertEqual(
                try jsonObject(in: uninstall.stdout)["label"] as? String, "com.updatebar.check")
        }
        ...
        func testBackgroundStatusHumanOutputShowsLabelAndPath() throws {
            ...
            let missing = try CLIProcess.run(
                ["background", "status"], home: home, environment: ["HOME": home.path])
            XCTAssertEqual(missing.exitCode, 0)
            XCTAssertEqual(missing.stderr, "")
            XCTAssertTrue(missing.stdout.contains("STATUS\tLABEL\tPATH"))
            XCTAssertTrue(
                missing.stdout.contains("not_installed\tcom.updatebar.check\t\(plistPath)"))
        ...
        func testBackgroundInstallAndUninstallHumanOutputShowsLabelAndPath() throws {
            ...
            let uninstall = try CLIProcess.run(
                ["background", "uninstall"], home: home, environment: ["HOME": home.path])
            XCTAssertEqual(uninstall.exitCode, 0)
        ...
        func testBackgroundInstallAndUninstallHumanOutputShowsManualLaunchctlNextSteps() throws {
            ...
            XCTAssertTrue(
                install.stdout.contains("launchctl bootstrap gui/$(id -u) \(quotedPlistPath)"))
            ...
            XCTAssertTrue(
                uninstall.stdout.contains("launchctl bootout gui/$(id -u)/com.updatebar.check"))
        }
        ...
        func testBackgroundJSONCommandsWithJSONStreamEqualsProduceGuidance() throws {
            ...
            let cases = [
                (
                    "background install", ["background", "install", "--json-stream=true"],
                    "Run updatebar background install --yes --json"
                ),
                (
                    "background status", ["background", "status", "--json-stream=true"],
                    "Run updatebar background status --json"
                ),
                (
                    "background uninstall", ["background", "uninstall", "--json-stream=true"],
                    "Run updatebar background uninstall --json"
                ),
            ]

            for (command, arguments, guidance) in cases {
                let result = try CLIProcess.run(
                    arguments, home: home, environment: ["HOME": home.path])
                ...
                XCTAssertTrue(
                    result.stdout.contains("\(command) does not support JSONL streaming"), command)
