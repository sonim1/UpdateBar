    ...
    final class DashboardSidebarViewControllerTests: XCTestCase {
        ...
        func testUpdateQueueRendersOneBoundedSummaryButton() throws {
            ...
            let bezel = try XCTUnwrap(button.cell).drawingRect(forBounds: button.bounds)
            let textBounds = button.convert(title.bounds, from: title)
                .union(button.convert(detail.bounds, from: detail))
            XCTAssertLessThanOrEqual(bezel.minY, textBounds.minY)
            XCTAssertGreaterThanOrEqual(bezel.maxY, textBounds.maxY)
