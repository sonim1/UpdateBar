...
<?xml version="1.0"?><rss xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle"><channel><item><sparkle:version>$version</sparkle:version><sparkle:shortVersionString>$version</sparkle:shortVersionString><enclosure url="https://updates.updatebar.royjen.com/UpdateBar-${version}-macos-arm64.dmg" length="$(/usr/bin/ruby -e 's=File.lstat(ARGV.fetch(0)); exit 1 unless s.file? && !s.symlink?; print s.size' "$R/dist/updates/$name")" sparkle:edSignature="$sig" /></item></channel></rss>
XML
}
