...
- [x] Add matching prompt to README, home and macOS. Link official installation instructions; inspect OS/architecture/existing install, preserve configuration, verify version and doctor, exclude recipe approval and tracked-tool updates.
- [x] Add clipboard enhancement and responsive wrapping; update the landing contract to permit only this exact deferred local script while still rejecting other scripts.
...
Verification evidence lives in .omo/evidence/ai-install-prompt (not published).
