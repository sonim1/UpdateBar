...
management tasks. Agent-safe `add` and `edit` paths are visible. Import/export,
other advanced item-management, background, configuration, and support commands
remain callable directly, but are hidden from default root help and shell
completions to keep the day-to-day surface small.
